...
const AdminCategory = () => {
  ...
  const selectedItem = (id: adminCategoryType["id"]) => {
    const filter: adminCategoryType[] | undefined = categorys?.filter(item => item.id === id)
  ...
  const addItem = () => {
    const newData: adminCategoryType = {
      "name": name.current.value,
      "description": desc.current.value,
      "fileId": 0,
    }

    axios.post(`${link}category/save?productUnits=KG`, newData, config)
      .then((res: AxiosResponse) => {
        setCategories(res.data.body)
        getCategory()
        openAdd()
  ...
  const editItem = (id: adminCategoryType["id"]) => {
  ...
  const deleteCategory = (id: adminCategoryType["id"]) => {
    axios.delete(`${link}category/${id}`, config)
      .then((res: AxiosResponse) => {
        setCategories(res.data.body)
        getCategory()
        openDelete()
  ...
  return (
    <AdminScreen title={"Categories"} footerTxt={"admin-controller"}>
      <div>
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="p-4 md:p-5">
                <div className="grid gap-4 mb-4 grid-cols-2">
                  ...
                  <div className="col-span-2">
                    <label for="price" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Describtion</label>
                    <input ref={desc} type="text" name="name" id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type product name" />
                  </div>
